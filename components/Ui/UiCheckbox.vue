...
<template>
    <div :class="{'error':error,'disabled':disabled}" class="checkbox-form">
        <input :id="'checkbox_form_item'+modelValue[optionValue]" type="checkbox" @change="toggleCheckbox">
        <label :for="'checkbox_form_item'+modelValue[optionValue]" class="checkbox-form__item">
            ...
            <span>{{ modelValue[optionLabel] }}</span>
        </label>
    </div>
</template>
